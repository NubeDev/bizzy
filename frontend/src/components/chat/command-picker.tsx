...
import type { PickerItem } from '@/hooks/use-command-picker'
...
export function CommandPicker({ items, grouped, selectedIndex, onSelect }: Props) {
  ...
  return (
    ...
    >
      {Object.entries(grouped).map(([appName, groupItems]) => (
        <div key={appName}>
            ...
            return (
              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="text-xs text-muted-foreground truncate leading-relaxed">{item.description}</div>
                  {item.arguments && item.arguments.length > 0 && (
                    <div className="flex gap-1 mt-0.5 flex-wrap">
                      {item.arguments.map(a => (
                        <span key={a.name} className={`text-[9px] font-mono px-1 py-0 border ${a.required ? 'border-primary/40 text-primary' : 'border-border text-muted-foreground'}`}>
                          {a.name}{a.required ? '*' : ''}
                        </span>
                      ))}
                    </div>
