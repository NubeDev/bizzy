  ...
  if (message.role === 'user') {
    return (
      <div className={`flex items-start ${gap}`}>
        <div className={`${avatarSize} rounded-none bg-primary flex items-center justify-center shrink-0 mt-0.5`}>
  ...
  return (
    <div className={`flex items-start ${gap} group`}>
      ...
      <div className="flex-1 min-w-0 space-y-1 pt-0.5">
        {/* Status indicator — only when streaming with no content yet */}
        ...
        {/* Extra content slot (e.g. file badges from builder) */}
        {extraContent}
      </div>
    </div>
  )
