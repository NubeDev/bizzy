interface Props {
  ...
  /** Subtitle text */
  subtitle?: string
  /** Custom icon rendered above the title */
  icon?: React.ReactNode
  /** Compact mode — smaller padding, single column */
  compact?: boolean
}
...
export function ChatSuggestions({
  ...
  return (
    <div className={`flex flex-col items-center justify-center h-full px-4 ${compact ? 'py-8' : ''}`}>
      ...
      <div className={`${compact ? 'space-y-2 w-full' : 'flex flex-wrap gap-2 justify-center max-w-lg'}`}>
        {suggestions.map((s) => (
          <button
            key={s.label}
            ...
            className={`text-left border border-border bg-card hover:bg-accent transition-colors text-muted-foreground hover:text-foreground ${
              compact
                ? 'w-full p-2.5 text-[11px] leading-relaxed'
                : 'px-3 py-2 text-xs'
            }`}
          >
            {s.label}
